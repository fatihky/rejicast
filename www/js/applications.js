                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var category = data.nodes[0].node.field_kategorisi.indexOf("Stand hostesi") != -1;
                                    ...
                                    $("#contentHolder.applications").append(profile);
                                    if (category) {
                                        $("#contentHolder.applications").append('</div><div class="singleProfileLabel">Göğüs:</div><div class="singleProfileContent">' + data.nodes[0].node.field_gogus + '</div><div class="singleProfileLabel">Bel:</div><div class="singleProfileContent">' + data.nodes[0].node.field_bel + '</div><div class="singleProfileLabel">Kalça:</div><div class="singleProfileContent">' + data.nodes[0].node.field_kalca);
                                    }
                                    $("#contentHolder.applications").append('<div class="singleProfileLabel">Ayak no:</div><div class="singleProfileContent">' + data.nodes[0].node.field_ayak_no + '</div><div class="singleProfileLabel">Göz rengi:</div><div class="singleProfileContent">' + data.nodes[0].node.field_goz_rengi + '</div><div class="singleProfileLabel">Beceriler:</div><div class="singleProfileContent">' + data.nodes[0].node.field_beceriler + '</div>');
                                    window.scrollTo(0,0);
                                    if (data.nodes[0].node.field_fotograflar.src !== undefined) {
                                        $("#contentHolder.applications").append('<img class="singleProfileOtherPics" src="' + data.nodes[0].node.field_fotograflar.src + '"/>'); 
                                    } else {
                                        $.each(data.nodes[0].node.field_fotograflar, function () {
                                            $("#contentHolder.applications").append('<img class="singleProfileOtherPics" src="' + this.src + '"/>');
                                        });
                                    }
                                    if (data.nodes[0].node.field_videolar) {
                                        $("#contentHolder.applications").append('Videolar');
                                        $.each(data.nodes, function() {
                                            $("#contentHolder.applications").append('<a href='+this.node.field_videolar+'><img class="videotn" src='+this.node.field_videolar_thumbnail_path+'/>');
                                        });
                                    }
